...
export const handleStartWhatsAppPreview = async ({
  ...
  let previewCredentials: MetaCredentials;
  let usingOwnCredentials = false;
  ...
  } else if (existingTypebot.whatsAppCredentialsId) {
    const dbCredentials = await getCredentials(
      existingTypebot.whatsAppCredentialsId,
      existingTypebot.workspaceId,
    );
    if (!dbCredentials)
      throw new ORPCError("BAD_REQUEST", {
        message: "Could not find WhatsApp credentials for this typebot",
      });
  ...
  // When using the bot's own credentials the session ID includes the phone
  // number ID so the production webhook can compute and route to it.
  const sessionId = usingOwnCredentials
    ? `${WHATSAPP_PREVIEW_SESSION_ID_PREFIX}${previewCredentials.phoneNumberId}-${to}`
    : `${WHATSAPP_PREVIEW_SESSION_ID_PREFIX}${to}`;
  ...
  return withSessionStore(sessionId, async (sessionStore) => {
    ...
    try {
      ...
      return {
        message: usingOwnCredentials
          ? "Session started. Send any message to the bot on WhatsApp to begin."
          : "Sent WA template",
      };
    } catch (error) {
